...
def doDownloadClasses():
    ...
    class MyDialog:
        ...
        def __init__(self, parent):
            ...
            self.myLabel2 = Label(top,
                                  text='Enter Location to Save Classes: Documents/')
            self.myLabel2.pack()
            self.myEntryBox2 = Entry(top)
            self.myEntryBox2.pack()
            self.mySubmitButton = Button(top, text='Update',
...
def doUpdateClasses():
    ...
    class MyDialog:
        ...
        def __init__(self, parent):
            top = self.top = Toplevel(parent)
            self.myLabel2 = Label(top,
                                  text='Enter Location of File: Documents/')
            self.myLabel2.pack()
            self.myEntryBox2 = Entry(top)
            self.myEntryBox2.pack()
            self.mySubmitButton = Button(top, text='Update',
...
def doUpdateClassWorth():
    ...
    class MyDialog:
        ...
        def __init__(self, parent):
            ...
            mySubmitButton = Button(top, text='Update',
                                    command=lambda: self.send())
        ...
        def send(self):
            def find_in_grid(frame, row, column):
                for children in frame.children.values():
                    ...
                    if info['row'] == str(row) and info['column'] == str(
                            column):
                        return children
                return None
    ...
    count = 0
    for cl in classes:
        cur.execute('UPDATE Classes SET Worth = ? WHERE ShortName = ?',
                    (float(worths[count]), cl[0]))
        count = count + 1
    ...
    class MyDialog:
        ...
        def __init__(self, parent):
            ...
            self.myLabel = Label(top,
                                 text='Remember to rename survey response to \'students.tsv\'!')
            self.myLabel.pack()
...
def doListClasses():
    ...
    class MyDialog:
        ...
        def __init__(self, parent):
            ...
            for item in classes:
                addtolist(item=item)
            self.mySubmitButton = Button(top, text='Finished',
                                         command=self.send)
            self.mySubmitButton.pack()
...
def doAutomateFast():
    ...
    d.set(message)
    mat_sch = auto.gen_sec_stu_matrix(pop=1000, keep=1, mats=10, output=output)[
        0]
...
def doAutomateBest():
    ...
    d.set(message)
    mat_sch = \
    auto.gen_sec_stu_matrix(pop=10000, keep=1, mats=100, output=output)[0]
    auto.updateDatabase(mat_sch, output)
    message = "Schedule Created!"
    d.set(message)
    return ()
        ...
        if len(cl[3]) > 1:
            for c in cl[3]:
                try:
                    ...
                    block_in_Calendar(text=cl[2] + " (" + name + ")",
                                      open=cl[1], day=c, start=cl[4], end=cl[5],
            ...
            try:
                ...
                block_in_Calendar(text=cl[2] + " (" + name + ")", open=cl[1],
                                  day=cl[3], start=cl[4], end=cl[5],
                                  calendarFrame=calendarFrame)
            except:
                continue
        # info.append(cl[2] + ": " + name + " " + cl[4] + "-" + cl[5])
...
def doViewStudent(student):
    ...
    for ut in unavail:
        block_in_Calendar(text='', open=3, day=ut[0], start=ut[1], end=ut[2],
                          calendarFrame=calendarFrame)
    ...
    prefer = cur.fetchall()
    for pt in prefer:
        block_in_Calendar(text='', open=2, day=pt[0], start=pt[1], end=pt[2],
                          calendarFrame=calendarFrame)
    cur.execute(
        'SELECT Year, Division, Skill FROM Students WHERE StudentID = ?',
        (student,))
    info = cur.fetchone()
    add_info_Calendar(text="Year: " + str(info[0]) + ", Div: " + str(
        info[1]) + ', Skill: ' + str(info[2]), calendarFrame=calendarFrame)

    sch = cur.execute('SELECT Scheduled FROM Students WHERE StudentID = ?',
                      (student,))
    sch = cur.fetchone()
    if sch[0] > 0:
        ...
        for cl in sch_classes:
            if len(cl[3]) > 1:
                for c in cl[3]:
                    try:
                        block_in_Calendar(text=cl[1] + " " + cl[2], open=1,
                                          day=c, start=cl[4], end=cl[5],
                                          calendarFrame=calendarFrame)
                    except:
                        continue
            else:
...
def centerselect():  # View
    ...
    if scheduling == 'class':
        ...
        if current == "any":
            ...
            cla = cur.execute('SELECT ClassID FROM Classes WHERE ShortName = ?',
                              (current_class,))
            cla = cur.fetchone()[0]
            sec1 = cur.execute(
                'SELECT SectionID FROM Sections WHERE ClassID = ? AND Name = ?',
                (cla, current_section))
            global sec
            sec = cur.fetchone()[0]

        students = cur.execute(
            'SELECT DISTINCT Name, StudentID, Scheduled FROM Students WHERE Scheduled < 0.9')  # ie less than fully scheduled
        ...
        for i in range(len(students)):
            if current_section != "any":
                ...
                student_tuples.append((int(globalvars.mat_prefs[stuID, colnum]),
                                       (students[i][1], students[i][0],
                                        students[i][2]),
                                       i))  # update from i to stuindex
            else:
                student_tuples.append((int(0), (
                students[i][1], students[i][0], students[i][2]), i))
        # sort p in order of highest value first
        openListbox.delete(0, END)
        if current_section != "any":
            student_tuples = sorted(student_tuples,
                                    key=lambda student: student[0],
                                    reverse=True)
        for item in student_tuples:
            item = str(item[0]) + "; " + str(item[1][1]) + " (" + str(
                item[1][2]) + ")"
            openListbox.insert(END, item)
        openListbox.insert(END, "undergrad")

        students = cur.execute(
            'SELECT DISTINCT Name, StudentID, Scheduled FROM Students WHERE Scheduled > 0.9')  # ie fully scheduled
        students = cur.fetchall()
        global mat_prefs
        student_tuples = []
        for i in range(len(students)):
...
def get_class_value(SectionID):
    ...
    conn = sqlite3.connect(globalvars.database_path)
    cur = conn.cursor()
    cla = cur.execute('''SELECT Worth FROM Sections WHERE SectionID = ?''',
                      (SectionID,))
    cla = cur.fetchone()[0]
    return (cla)
...
def openaddselect():  # Schedule
    ...
    global mat_no  # GMS
    global mat_add  # GMS
    global section_index  # GMS
    global current_section  # GMS
    if scheduling == 'class':
        print 'add-class'
        ...
        conn = sqlite3.connect(globalvars.database_path)
        cur = conn.cursor()
        stu = cur.execute('SELECT StudentID FROM Students WHERE Name = ?',
                          (current,))
        stu = cur.fetchone()[0]
        # global student_index  # GMS
        stuID = student_index[stu]
        ...
        if current_section != "any":
            ...
            oldvalue = cur.execute(
                'SELECT Scheduled FROM Students WHERE StudentID = ?', (stu,))
            oldvalue = cur.fetchone()[0]
            c = cur.execute(
                'UPDATE Students SET Scheduled = ? WHERE StudentID = ?',
                (float(value) + float(oldvalue), stu))
            c = cur.execute(
                'UPDATE Sections SET Scheduled = ? WHERE SectionID = ?',
                (1, sec))
            c = cur.execute(
                'UPDATE Sections SET StudentID = ? WHERE SectionID = ?', (
                stu, sec))  # wonder why this doesn't work with an AND statement
            conn.commit()
        ...
        if current_section == "any":
            ...
            # global mat_no  # GMS
            cur.execute(
                'SELECT A.SectionID FROM Sections A INNER JOIN Classes B ON A.ClassID = B.ClassID WHERE B.ShortName = ?',
                (current_class,))
            secs = cur.fetchall()
            # global section_index  # GMS
            for sec in secs:
            ...
            c = cur.execute(
                'UPDATE Students SET Scheduled = ? WHERE StudentID = ?',
                (float(oldvalue) + float(value), stu))
            conn.commit()
...
def openremoveselect():  # Remove
    ...
    if scheduling == 'student':
        ...
        if current_section != "any":
            ...
            c = cur.execute(
                'UPDATE Sections SET StudentID = ? WHERE SectionID = ?',
                (0, sec))  # wonder why this doesn't work with an AND statement
...
def openaddblockselect():  # Add Block
    ...
    if scheduling == 'class':
        ...
        if current_section == 'any':
            ...
            cur.execute('SELECT ClassID FROM Classes WHERE ShortName = ?',
                        (current_class,))
...
def doOpenSchedule(output2=None):
    ...
    globalvars.mat_prefs = np.load(
        output + "/mat_prefs.npy")  # matrices.matrix_pref()
    global section_index
    section_index = matrices.section_index()
    global student_index
    student_index = matrices.student_index()
...
bselect = Button(buttonSelectFrame, text="Select",
                 command=lambda: leftselect())  # lambda necessary to prevent call upon opening
...
b2Remove = Button(buttonFrame2, text="Remove",
                  command=lambda: openremoveselect())
b2Remove.pack(side=TOP)
b3Remove = Button(buttonFrame2, text="Add Block",
                  command=lambda: openaddblockselect())
b3Remove.pack(side=TOP)
b4Remove = Button(buttonFrame2, text="Remove Block",
                  command=lambda: openremoveblockselect())
...
filemenu.add_separator()
filemenu.add_command(label="Export Email",
                     command=lambda: export.doExportMail(output2=output, d=d))
filemenu.add_command(label="Export Susan",
                     command=lambda: export.doExportSusan(output2=output, d=d))
filemenu.add_command(label="Export Linda",
                     command=lambda: export.doExportLinda(output2=output, d=d))
filemenu.add_command(label="Export All",
                     command=lambda: export.doExportAll(output2=output, d=d))
filemenu.add_separator()
...
## Update Menu
updatemenu.add_command(label="Download Classes",
                       command=lambda: doDownloadClasses())
updatemenu.add_command(label="Update Class Worths",
                       command=lambda: doUpdateClassWorth())
updatemenu.add_command(label="Update Classes",
                       command=lambda: doUpdateClasses())
updatemenu.add_command(label="Update Students",
                       command=lambda: doUpdateStudents())

## Google Survey Meny
googlemenu.add_command(label="List Classes", command=lambda: doListClasses())
googlemenu.add_command(label="List Professors",
                       command=lambda: doListProfessors())
...
globalvars.sec_sec_matrix_path = os.path.join(dir_path,
                                              'section_section_matrix.npy')
