...
def matrix_sections():
    ...
    for i in range(len(sections)):
        for j in range(len(sections)):
            ...
            b = sections[j][0]
            # time conflicts
            conflictsa = cur.execute(
                '''SELECT COUNT(*) FROM Con_Section_Section WHERE SectionID1 = ? AND SectionID2 = ?''',
                (a, b))
            conflictsa = cur.fetchall()
            conflictsa = conflictsa[0][0]
            conflictsb = cur.execute(
                '''SELECT COUNT(*) FROM Con_Section_Section WHERE SectionID1 = ? AND SectionID2 = ?''',
                (b, a))
            ...
            # same class name
            prefa = cur.execute(
                '''SELECT COUNT(*) FROM Pref_Section_Section WHERE SectionID1 = ? AND SectionID2 = ?''',
                (a, b))
            ...
            # same professor
            pref2a = cur.execute(
                '''SELECT COUNT(*) FROM Pref2_Section_Section WHERE SectionID1 = ? AND SectionID2 = ?''',
                (a, b))
    ...
    for i in range(len(students)):
        ...
        for j in range(len(sections)):
            ...
            conflict3 = cur.execute('''SELECT COUNT(*)
							...
							WHERE A.StudentID = ? AND B.SectionID = ?''',
                                    (students[i][0], sections[j][0]))
            ...
            print "GMS testing of fix in matrices.py"
            print "prefsk fetch: ", prefsk
            print "globalvars.stu_skill.pref fetch: ", globalvars.stu_skill_pref
            prefsk = prefsk * globalvars.stu_skill.pref
            print "prefsk is now: ", prefsk
            ...
            mat_prefs[
                i, j] = conflict1 + conflict2 + conflict3 + conflict4 + pref1 + pref2 + pref3 + pref4 + prefyr + prefdiv + prefsk
